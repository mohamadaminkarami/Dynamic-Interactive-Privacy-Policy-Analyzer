...
class TextSegment(BaseModel):
    ...
    sensitivity_score: float = Field(
        ..., ge=0.0, le=10.0, description="Sensitivity score for this segment"
    )
    ...
    highlight_color: str = Field(
        default="neutral", description="Background highlight color"
    )
    ...
    requires_attention: bool = Field(
        default=False, description="Whether this segment requires special attention"
    )

    # Contextual information
    context_type: str = Field(
        default="general",
        description="Type of content: data_collection, sharing, rights, etc.",
    )
    key_terms: List[str] = Field(
        default_factory=list, description="Key terms that triggered high sensitivity"
    )
...
class StyledContent(BaseModel):
    ...
    segments: List[TextSegment] = Field(
        default_factory=list, description="Text segments with styling"
    )
    overall_sensitivity: float = Field(
        ..., ge=0.0, le=10.0, description="Overall sensitivity of the content"
    )
    styling_applied: bool = Field(
        default=False, description="Whether styling has been applied"
    )

    # Styling statistics
    high_sensitivity_count: int = Field(
        default=0, description="Number of high-sensitivity segments"
    )
    medium_sensitivity_count: int = Field(
        default=0, description="Number of medium-sensitivity segments"
    )
...
class QuizOption(BaseModel):
    ...
    explanation: Optional[str] = Field(
        None, description="Explanation for why this is/isn't correct"
    )
...
class QuizQuestion(BaseModel):
    ...
    question_type: str = Field(
        ...
        description="Type: multiple_choice, true_false, fill_blank",
    )
    ...
    related_content: str = Field(
        ..., description="The original content this question is based on"
    )
    sensitivity_score: float = Field(
        ..., ge=0.0, le=10.0, description="Sensitivity score of related content"
    )
    learning_objective: str = Field(
        ..., description="What the user should learn from this question"
    )
...
class InteractiveQuiz(BaseModel):
    ...
    section_id: str = Field(
        ..., description="ID of the policy section this quiz belongs to"
    )
    ...
    estimated_time_minutes: int = Field(
        default=2, description="Estimated completion time"
    )
    ...
    sensitivity_threshold: float = Field(
        default=8.0, description="Minimum sensitivity score that triggered this quiz"
    )
    created_at: datetime = Field(
        default_factory=datetime.now, description="Quiz creation timestamp"
    )

    # Learning outcomes
    learning_objectives: List[str] = Field(
        default_factory=list, description="What users will learn"
    )
    key_takeaways: List[str] = Field(
        default_factory=list, description="Important points to remember"
    )
...
class QuizResult(BaseModel):
    ...
    answers: Dict[str, str] = Field(
        default_factory=dict, description="Question ID to selected answer ID mapping"
    )
...
class UserImpactAnalysis(BaseModel):
    ...
    sensitivity_score: float = Field(
        ..., ge=0.0, le=10.0, description="Numerical sensitivity score (0-10)"
    )
    privacy_impact_score: float = Field(
        ..., ge=0.0, le=10.0, description="Impact on user privacy (0-10)"
    )
    data_sharing_risk: float = Field(
        ..., ge=0.0, le=10.0, description="Risk of data sharing/misuse (0-10)"
    )

    # Existing scores (1-5)
    user_control: int = Field(
        ..., ge=1, le=5, description="Level of user control (1-5)"
    )
    transparency_score: int = Field(
        ..., ge=1, le=5, description="Transparency score (1-5)"
    )

    # Content categorization
    key_concerns: List[str] = Field(
        default_factory=list, description="Key concerns for users"
    )
    actionable_rights: List[UserRight] = Field(
        default_factory=list, description="Rights users can exercise"
    )

    # Advanced UI features
    engagement_level: str = Field(
        default="standard",
        description="UI engagement level: standard, interactive, quiz",
    )
    requires_quiz: bool = Field(
        default=False, description="Whether this section needs an interactive quiz"
    )
    requires_visual_aid: bool = Field(
        default=False, description="Whether this section needs AI-generated images"
    )
    text_emphasis_level: int = Field(
        default=1, ge=1, le=5, description="Text emphasis level for styling (1-5)"
    )

    # Visual styling hints
    highlight_color: str = Field(
        default="neutral",
        description="Suggested highlight color: neutral, yellow, orange, red",
    )
    font_weight: str = Field(
        default="normal", description="Suggested font weight: normal, medium, bold"
    )
...
class ProcessedSection(BaseModel):
    ...
    styled_content: Optional[StyledContent] = Field(
        None, description="Content with sensitivity-based styling"
    )
    styled_summary: Optional[StyledContent] = Field(
        None, description="Summary with sensitivity-based styling"
    )
    ...
    quiz: Optional[InteractiveQuiz] = Field(
        None, description="Interactive quiz for high-sensitivity sections (8+)"
    )
    requires_quiz: bool = Field(
        default=False, description="Whether this section requires a quiz"
    )

    # Extracted information
    data_types: List[DataType] = Field(
        default_factory=list, description="Types of data mentioned"
    )
    user_rights: List[UserRight] = Field(
        default_factory=list, description="User rights mentioned"
    )
    ...
    # Legal framework
    legal_frameworks: List[LegalFramework] = Field(
        default_factory=list, description="Applicable legal frameworks"
    )

    # Enhanced scoring metadata
    importance_score: float = Field(
        ..., ge=0.0, le=1.0, description="Importance score for ranking"
    )
...
class PrivacyPolicyDocument(BaseModel):
    ...
    # Content
    sections: List[ProcessedSection] = Field(
        default_factory=list, description="Processed sections"
    )

    # Legacy analysis (backward compatibility)
    overall_risk_level: RiskLevel = Field(..., description="Overall risk level")
    user_friendliness_score: int = Field(
        ..., ge=1, le=5, description="User friendliness score"
    )

    # Enhanced numerical analysis
    overall_sensitivity_score: float = Field(
        default=0.0, ge=0.0, le=10.0, description="Overall sensitivity (0-10)"
    )
    overall_privacy_impact: float = Field(
        default=0.0, ge=0.0, le=10.0, description="Overall privacy impact (0-10)"
    )
    compliance_score: float = Field(
        default=0.0,
        ge=0.0,
        le=10.0,
        description="Compliance with privacy standards (0-10)",
    )
    readability_score: float = Field(
        default=0.0, ge=0.0, le=10.0, description="How easy to understand (0-10)"
    )
    ...
    high_risk_sections: int = Field(
        default=0, description="Number of high-risk sections"
    )
    interactive_sections: int = Field(
        default=0, description="Number of sections requiring interaction"
    )
