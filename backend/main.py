...
class WebpageGenerationRequest(BaseModel):
    ...
    custom_options: Optional[Dict[str, Any]] = Field(default_factory=dict, description="Custom options")
...
app.mount("/static/webpages", StaticFiles(directory="generated_webpages"), name="generated_webpages")
...
async def generate_webpage(request: WebpageGenerationRequest):
    ...
    try:
        ...
        from .models import UserImpactAnalysis, DataType, UserRight
        ...
        sections = [
            ProcessedSection(
                ...
                user_impact=UserImpactAnalysis(
                    ...
                    key_concerns=["Data retention period unclear", "Third-party sharing"],
                ...
                user_rights=[UserRight.ACCESS, UserRight.DELETION, UserRight.PORTABILITY],
        ...
        raise HTTPException(status_code=500, detail=f"Webpage generation failed: {str(e)}")
...
async def serve_webpage(webpage_id: str):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to serve webpage: {str(e)}")
    ...
    try:
        ...
        (function() {{
            ...
            if (container) {{
                ...
                iframe.src = '{config.APP_VERSION}/embed/iframe/{webpage_id}';
...
async def get_webpage_info(webpage_id: str):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to retrieve webpage info: {str(e)}")
...
async def list_generated_webpages():
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to list webpages: {str(e)}")
...
async def analyze_and_generate_webpage(request: PolicyProcessingRequest):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Analysis and generation failed: {str(e)}")
